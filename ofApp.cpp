	...
	for (int x = len * -0.5; x <= len * 0.5; x += span) {
		...
		for (int y = len * -2; y <= len * 0.5; y += span) {
			...
			for (int z = len * -0.5; z <= len * 0.5; z += span) {
				...
				if ((-110 <= x && x <= 110) && (-110 <= z && z <= 110)) { continue; };

				auto noise_value = ofNoise(x * 0.015, z * 0.015, y * 0.0025 + ofGetFrameNum() * 0.025);
				if (y > len * 0.4) { noise_value = 1; }

				if (noise_value > 0.55) {

					this->setBoxToMesh(this->face, this->frame, glm::vec3(x, y, z), span, ofColor(0), ofColor(39, 239, 39));
				}
			}
		}
	}
